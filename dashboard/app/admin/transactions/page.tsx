...
async function fetchData(): Promise<{ transactions: Transaction[]; businesses: Business[] }> {
  try {
    ...
    return { transactions: [], businesses: [] };
  }
}

async function AdminTransactionsList() {
  const { transactions, businesses } = await fetchData();
    ...
    <>
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-brand-ink-surface border border-brand-ink-surface-2 rounded-xl p-4">
          ...
          <p className="text-xl font-bold text-white font-display">{transactions.length}</p>
        </div>
        <div className="bg-brand-ink-surface border border-brand-ink-surface-2 rounded-xl p-4">
      ...
      <div className="bg-brand-ink-surface border border-brand-ink-surface-2 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-brand-ink-surface-2 flex items-center justify-between">
          <h3 className="font-semibold text-white font-display">All Platform Transactions</h3>
                ...
                transactions.map((txn) => {
                  ...
                  return (
                    <tr key={txn.id} className="table-row-hover">
                      ...
                      <td className="px-6 py-3.5">
                        <span className="text-xs font-mono text-gray-500 bg-brand-ink-surface-2 px-1.5 py-0.5 rounded">
