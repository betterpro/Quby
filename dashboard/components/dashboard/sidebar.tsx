  ...
  return (
    <aside className="w-64 min-h-screen bg-brand-ink-bg border-r border-brand-ink-line flex flex-col fixed left-0 top-0 bottom-0 z-40">
      ...
      <div className="p-6 border-b border-brand-ink-line">
        <Logo size="sm" />
        <p className="text-xs text-brand-ink-dim mt-2 ml-0.5">Business Dashboard</p>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-all group ${
                isActive
                  ? "bg-brand-green/15 text-brand-green-bright border border-brand-green/20"
                  : "text-brand-ink-dim hover:text-brand-ink-text hover:bg-brand-ink-surface"
              }`}
            >
